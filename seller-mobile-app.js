(function sellerAppBootstrap() {
  var APP_NAME = "MatrixMarket Seller";
  var APP_ICON = "seller.png";
  var MANIFEST_HREF = "seller-app.webmanifest";
  var DISMISS_KEY = "mmSellerInstallPromptDismissedAt";
  var DISMISS_MS = 3 * 24 * 60 * 60 * 1000;
  var PROMPT_ID = "mmSellerInstallPrompt";
  var STYLE_ID = "mmSellerInstallPromptStyles";
  ...
  function ensureHeadTags() {
    ...
    setLink("manifest", MANIFEST_HREF);
    setLink("apple-touch-icon", APP_ICON);
  ...
  function ensureStyles() {
    ...
    style.textContent =
      ...
      "#" + PROMPT_ID + " .mm-install-primary{background:#10b981;color:#fff}" +
  ...
  function promptMessage() {
    if (deferredPrompt) {
      return {
        title: "Install Seller App",
        body: "Add the seller workspace to your phone for faster dashboard, order, and delivery access.",
        primary: "Install",
        secondary: "Later"
      };
    }
    return {
      title: "Add Seller App To Home Screen",
      body: "On iPhone, tap Share in Safari, then choose Add to Home Screen to install the seller app.",
      primary: "Got It",
      secondary: "Later"
    };
  }
  ...
  function showPrompt() {
    ...
    root.innerHTML =
      ...
      '<div class="mm-install-copy"><strong>' + copy.title + '</strong><span>' + copy.body + '</span></div>' +
  ...
  if (document.readyState === "loading") document.addEventListener("DOMContentLoaded", bootPrompt, { once: true });
  else bootPrompt();
})();
